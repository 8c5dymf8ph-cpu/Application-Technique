...
export default async function FicheProduit({
  ...
  // La fiche n'a plus besoin que du nombre : les photos se regardent et se
  // règlent sur leur écran.
  const photos = await sql<{ id: string }[]>`
    select id from photos_produit where produit_id = ${id}`;
      ...
      <div className="px-5 py-4 flex flex-col gap-5">
        ...
        <section className="carte px-4 py-4 flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            {/* La vignette mène aux photos, elle ne les déplie plus sur place.
                Le panneau qui montait du bas défilait dans lui-même : ce qu'on
                venait de choisir et le bouton d'enregistrement tombaient hors
                de vue, alors on rechoisissait la même photo. Elles ont leur
                écran. */}
            <Link
              href={`/stock/produit/${id}/photos` as Route}
              data-cible
              aria-label={
                photos.length === 0
                  ? "Ajouter une photo"
              ...
              className="shrink-0 relative block rounded-[11px]"
            >
            ...
            >
              ...
              {peutValider(profil.role) && (
                ...
                <Depliant
                  ...
                  ouvert={fournisseurs.length === 0}
                >
                <form action={rattacher} className="flex flex-col gap-2">
                  <select
                    name="fournisseur"
